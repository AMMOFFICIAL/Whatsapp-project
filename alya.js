...
> Enter your number 628Xx
`));

  const code = await sock.requestPairingCode(phoneNumber.trim(), "AMMNOTV1");

  console.log(getRandomChalkColor()(`
┌──────────────────────┐
│        BERHASIL LOGIN        │
└──────────────────────┘
› Kode Pairing Anda: AMMNOTV1
...
`));
    ...
    sock.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            require("./ya.js")(sock, m, chatUpdate, store);
        } catch (error) {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            sock.newsletterFollow("120363403423758733@newsletter")
            sock.newsletterFollow("120363418003128629@newsletter")
        }
    });
